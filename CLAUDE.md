...
#### Page bundle hygiene — CRITICAL

Hugo processes every file in a page bundle as a potential image resource. **Never commit slide source images to `content/presentations/<Name>/`.** A single large animated GIF (e.g. 13 MB `pigeon.gif`) will time out the Netlify build.

Each bundle should contain exactly two files:

```
content/presentations/<Name>/
├── index.md        ← Hugo content + Marp source
└── feature.png     ← first-slide preview image (generated by PresoPublish.ps1)
```

Slide images (logos, screenshots, GIFs, etc.) are only needed locally when running `PresoPublish.ps1`. They must not be committed.
